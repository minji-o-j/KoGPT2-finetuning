	...
	for epoch in range(epoch):
		for data in data_loader:
			...
			if (count > 0 and count % 1000 == 0) or (len(data) < batch_size):
				sent = sample_sequence(model.to("cpu"), tok,vocab, sent="여름", text_size=100, temperature=0.7, top_p=0.8, top_k=40)
			...
			if (count > 0 and count % 100000 == 0) or (len(data) < batch_size):
				# 모델 저장
				try:
					torch.save({
						'epoch': epoch,
