...
driver.implicitly_wait(5) # 암묵적으로 웹 자원 로드를 위해 5초까지 기다려 준다.
    ...
    for i in range(2,46986): #tag 2~46985까지 있었음
        ...
        for j in page:
            ...
            for ply in ply_name:
                ...
                ply_delete=ply_delete.lower()
                ply_delete=ply_delete.translate({ord('♬'): '',ord('♪'): '',ord('!'): '', ord('~'): '', ord(','): '', ord('\''): '',ord('_'): ' ' }) # '!', ',' 제거, _는 띄어쓰기로 바꿈
                ply_delete=ply_delete.replace("\[playlist\]","playlist").replace("\<playlist\>","playlist").replace("\(playlist\)","playlist")
                ply_delete=ply_delete.replace("r&b","rnb")

                # 플레이리스트 이름이 너무 많기 때문에 위의 경우와 공백을 제외하고 특수문자가 있는 경우는 아예 list에 포함시키지 않을 예정
...
data.to_csv('playlist_repeat5.csv',encoding='utf-8-sig')
